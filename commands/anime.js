...
async function information (slug) {
	try {
		...
		let data = await response.json()
		return data
	} catch (error) { return new Error(err) }
}
...
export default {
    ...
    message.channel.send(question)
		...
		try {
			const collection = await message.channel.awaitMessages({ filter, max: 1, time: 15000, errors: ['time'] })
			...
			message.reply({ components: embeds.components, embeds: embeds.embeds })
